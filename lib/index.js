...
Client.prototype.downloadFile = function(params, opts = { appendExisting: true }) {
  var self = this;
  ...
  function doTheDownload(cb) {
    ...
    request.on('httpHeaders', function(statusCode, headers, resp) {
      ...
      if (headers['content-length'] == undefined) {
        ...
        try {
          fs.accessSync(localFile, fs.constants.R_OK | fs.constants.W_OK);
          localFileExists = true;
          startByte = fs.statSync(localFile).size;
        } catch (err) {
          localFileExists = false;
        }
        if (opts.appendExisting && localFileExists) {
          outStream = fs.createWriteStream(localFile, { flags: 'r+', start: startByte });
        } else {
          outStream = fs.createWriteStream(localFile);
        }
